...
class MuleAccountAlert:
    ...
    pattern_type: str  # 'classic_mule', 'periodic_mule', 'threshold_mule'
    detection_period: Dict[str, Any]
    collection_phase: Dict[str, Any]  # Small credits analysis
    disbursement_phase: Dict[str, Any]  # Large debits analysis
...
class MuleAccountDetector:
    ...
    def __init__(self):
        ...
        # Configuration parameters - now adaptive/statistical based
        self.config = {
            'min_collection_transactions': 5,  # Minimum credits to consider
            'min_disbursement_amount_ratio': 3.0,  # Large debit must be 3x average credit
            'max_collection_period_days': 30,  # Collection period window
            'small_percentile_threshold': 0.3,  # Bottom 30% of amounts considered "small"
            'large_percentile_threshold': 0.8,  # Top 20% of amounts considered "large"
            'velocity_threshold': 0.5,  # Transactions per day threshold
            'periodicity_tolerance': 2,  # Days tolerance for periodic patterns
            'collection_disbursement_ratio': 2.0,  # Credits should be 2x more frequent than debits
            'amount_variance_threshold': 2.0,  # High variance indicator for structuring
        }
    
    def detect_mule_patterns(self, df: pd.DataFrame, 
                           account_identifier: str = None) -> List[MuleAccountAlert]:
        """
        ...
        # PRIMARY PATTERN: Pass-through Mule (Inflow ≈ Outflow)
        passthrough_alert = self._detect_passthrough_mule_pattern(df_clean, account_identifier)
        if passthrough_alert:
            alerts.append(passthrough_alert)
        
        # SECONDARY PATTERNS (only if pass-through detected)
        if passthrough_alert:
            # Pattern 1: Classic Mule (Many small credits → Few large debits)
            classic_alert = self._detect_classic_mule_pattern(df_clean, account_identifier)
            if classic_alert:
                alerts.append(classic_alert)
            
            # Pattern 2: Periodic Mule (Regular disbursement cycles)
            periodic_alert = self._detect_periodic_mule_pattern(df_clean, account_identifier)
            if periodic_alert:
                alerts.append(periodic_alert)
            
            # Pattern 3: Threshold Mule (Avoiding reporting thresholds)
            threshold_alert = self._detect_threshold_mule_pattern(df_clean, account_identifier)
            if threshold_alert:
    ...
    def _prepare_mule_analysis_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add derived columns for mule analysis
        ...
        # Adaptive amount categorization based on account's own patterns
        if len(df_clean) > 0:
            # Calculate percentile-based thresholds for this specific account
            small_threshold = df_clean['amount'].quantile(self.config['small_percentile_threshold'])
            large_threshold = df_clean['amount'].quantile(self.config['large_percentile_threshold'])
            
            df_clean['is_small_amount'] = df_clean['amount'] <= small_threshold
            df_clean['is_large_amount'] = df_clean['amount'] >= large_threshold
            
            # Store adaptive thresholds for reporting
    ...
    def _analyze_multiple_time_intervals(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """
        ...
        # Interval 1: Overall account balance (lifetime)
        ...
        if total_credits > 0 or total_debits > 0:
            ...
            results.append({
                ...
                'suspicion_score': 1.0 - net_flow_ratio,  # Lower ratio = higher suspicion
                'periods_analyzed': 1,
                'description': 'Overall account balance'
            })
        
        # Interval 2: Daily balancing (sophisticated daily operations)
        daily_results = self._analyze_daily_balancing(df)
        if daily_results:
            results.extend(daily_results)
        
        # Interval 3: Weekly balancing (weekly cycles)
        weekly_results = self._analyze_weekly_balancing(df)
        if weekly_results:
            results.extend(weekly_results)
        
        # Interval 4: Monthly balancing (monthly cycles)
        monthly_results = self._analyze_monthly_balancing(df)
        if monthly_results:
            results.extend(monthly_results)
        
        # Interval 5: Rolling window analysis (sliding 7-day, 30-day windows)
    ...
    def _analyze_daily_balancing(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for date, day_df in daily_groups:
                ...
                # Only consider days with both credits and debits
                if day_credits > 0 and day_debits > 0:
                    ...
                    if day_total_flow > 0:
                        ...
                        if day_ratio <= 0.1:  # Balanced within 10%
                            balanced_days += 1
            
            # If significant portion of days are balanced, it's suspicious
            if total_days_with_both >= 3 and balanced_days >= total_days_with_both * 0.6:
                ...
                results.append({
                    ...
                    'net_flow_ratio': abs(df['CREDIT'].sum() - df['DEBIT'].sum()) / (df['CREDIT'].sum() + df['DEBIT'].sum()),
                    'suspicion_score': balance_ratio,  # Higher ratio = more suspicious for daily balancing
    ...
    def _analyze_weekly_balancing(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for week, week_df in weekly_groups:
                ...
                # Only consider weeks with both credits and debits
                if week_credits > 0 and week_debits > 0:
                    ...
                    if week_total_flow > 0:
                        ...
                        if week_ratio <= 0.15:  # Balanced within 15%
                            balanced_weeks += 1
            
            # If significant portion of weeks are balanced, it's suspicious
            if total_weeks_with_both >= 2 and balanced_weeks >= total_weeks_with_both * 0.6:
    ...
    def _analyze_monthly_balancing(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for month, month_df in monthly_groups:
                ...
                # Only consider months with both credits and debits
                if month_credits > 0 and month_debits > 0:
                    ...
                    if month_total_flow > 0:
                        ...
                        if month_ratio <= 0.2:  # Balanced within 20%
                            balanced_months += 1
            
            # If significant portion of months are balanced, it's suspicious
            if total_months_with_both >= 2 and balanced_months >= total_months_with_both * 0.6:
    ...
    def _analyze_rolling_windows(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            seven_day_results = self._analyze_rolling_window(df, window_days=7, window_name='7-day')
            if seven_day_results:
                results.extend(seven_day_results)
            
            # 30-day rolling windows
            thirty_day_results = self._analyze_rolling_window(df, window_days=30, window_name='30-day')
    ...
    def _analyze_rolling_window(self, df: pd.DataFrame, window_days: int, window_name: str) -> List[Dict[str, Any]]:
        """Analyze a specific rolling window size"""
        results = []
        
        try:
            if len(df) < 5:  # Need minimum transactions
                return results
            ...
            while current_date <= end_date:
                ...
                if len(window_df) >= 2:  # Need minimum transactions in window
                    ...
                    if window_credits > 0 and window_debits > 0:
                        ...
                        if window_total_flow > 0:
                            ...
                            if window_ratio <= 0.15:  # Balanced within 15%
                                balanced_windows += 1
                
                current_date += timedelta(days=3)  # Move window by 3 days
            
            # If significant portion of windows are balanced, it's suspicious
    ...
    def _detect_passthrough_mule_pattern(self, df: pd.DataFrame, 
                                       account_id: str = None) -> Optional[MuleAccountAlert]:
        """
        ...
        """
        try:
            # Multi-interval analysis for sophisticated mule detection
            interval_results = self._analyze_multiple_time_intervals(df)
            
            if not interval_results:
                return None
            
            # Get the most suspicious interval result
            best_result = max(interval_results, key=lambda x: x['suspicion_score'])
            
            # Use the best result for overall assessment
            ...
            # Calculate confidence based on how close to balanced the account is
            confidence_score = 0.0
            risk_indicators = []
            
            # Primary Indicator: Flow Balance (70% weight)
            # The closer to balanced (net_flow_ratio → 0), the higher the mule suspicion
            if net_flow_ratio <= 0.02:  # Within 2% balance - extremely suspicious
                confidence_score += 0.70
                risk_indicators.append(f"Extremely balanced flow: {net_flow_ratio*100:.1f}% net flow ratio (strong pass-through indicator)")
            elif net_flow_ratio <= 0.05:  # Within 5% balance - highly suspicious
                confidence_score += 0.60
                risk_indicators.append(f"Highly balanced flow: {net_flow_ratio*100:.1f}% net flow ratio (pass-through indicator)")
            elif net_flow_ratio <= 0.10:  # Within 10% balance - suspicious
                confidence_score += 0.45
                risk_indicators.append(f"Balanced flow: {net_flow_ratio*100:.1f}% net flow ratio (potential pass-through)")
            elif net_flow_ratio <= 0.20:  # Within 20% balance - moderately suspicious
                confidence_score += 0.25
                risk_indicators.append(f"Moderately balanced flow: {net_flow_ratio*100:.1f}% net flow ratio")
            
            # Secondary Indicator: Transaction Volume (15% weight)
            # Higher volume increases suspicion for pass-through activity
            transaction_count = len(df)
            if transaction_count >= 15:
                confidence_score += 0.15
                risk_indicators.append(f"High transaction volume: {transaction_count} transactions")
            elif transaction_count >= 8:
                confidence_score += 0.10
                risk_indicators.append(f"Moderate transaction volume: {transaction_count} transactions")
            elif transaction_count >= 5:
                confidence_score += 0.05
                risk_indicators.append(f"Sufficient transaction volume: {transaction_count} transactions")
            
            # Tertiary Indicator: Time Span (10% weight)
            # Concentrated activity over short periods is more suspicious
            date_span = (df['DATE'].max() - df['DATE'].min()).days
            if date_span <= 30 and transaction_count >= 8:
                confidence_score += 0.10
                risk_indicators.append(f"Concentrated activity: {transaction_count} transactions in {date_span} days")
            elif date_span <= 60 and transaction_count >= 12:
                confidence_score += 0.05
                risk_indicators.append(f"Active period: {transaction_count} transactions in {date_span} days")
            
            # Quaternary Indicator: Both Credits and Debits Present (5% weight)
            # True mule accounts have both significant inflows and outflows
            credits_count = len(df[df['CREDIT'] > 0])
            debits_count = len(df[df['DEBIT'] > 0])
            
            if credits_count >= 3 and debits_count >= 2:
                confidence_score += 0.05
                risk_indicators.append(f"Bidirectional flow: {credits_count} credits and {debits_count} debits")
            elif credits_count >= 2 and debits_count >= 1:
                confidence_score += 0.02
                risk_indicators.append(f"Mixed transactions: {credits_count} credits and {debits_count} debits")
            ...
            # Only create alert if confidence is above threshold AND it's truly balanced
            # Require both sufficient confidence AND low net flow ratio for mule classification
            if adjusted_confidence >= 0.4 and net_flow_ratio <= 0.25:  # Must be reasonably balanced
                ...
                analysis_data = {
                    ...
                    'flow_balance_score': 1.0 - net_flow_ratio,  # Higher score = more balanced
                    'pass_through_indicator': 'HIGH' if net_flow_ratio <= 0.05 else 'MEDIUM' if net_flow_ratio <= 0.15 else 'LOW',
                    'detection_interval': interval_type,
                    'interval_analysis': best_result.get('description', 'Overall account analysis'),
                    ...
                    'intervals_summary': [
                        ...
                        } for r in interval_results[:5]  # Top 5 most suspicious
                    ]
                }
                ...
                return MuleAccountAlert(
                    ...
                    collection_phase={'note': 'Pass-through analysis - see disbursement_phase for details'},
                    disbursement_phase=analysis_data,
                    risk_indicators=risk_indicators,
                    recommended_actions=self._generate_passthrough_recommendations(adjusted_confidence, net_flow_ratio)
                )
    ...
    def _generate_passthrough_recommendations(self, confidence_score: float, 
                                            net_flow_ratio: float) -> List[str]:
        """Generate specific recommendations for pass-through mule accounts"""
        recommendations = []
        
        if net_flow_ratio <= 0.05:  # Highly balanced
            recommendations.append("🚨 CRITICAL: Highly balanced inflow/outflow indicates pass-through money laundering")
            ...
            recommendations.append("👮 NOTIFY: Law enforcement - potential money laundering operation")
        elif net_flow_ratio <= 0.15:  # Moderately balanced
            recommendations.append("⚠️ HIGH RISK: Balanced flow pattern suggests mule account activity")
            recommendations.append("🔍 INVESTIGATE: Review all counterparties and transaction purposes")
            recommendations.append("📊 MONITOR: Enhanced transaction monitoring required")
            recommendations.append("📋 CONSIDER: SAR filing based on additional factors")
        else:
            recommendations.append("👀 MONITOR: Potential pass-through activity detected")
            recommendations.append("📊 ANALYZE: Review transaction patterns and counterparties")
        
        recommendations.append("🔗 CROSS-REFERENCE: Check against known mule account networks")
        recommendations.append("👥 INVESTIGATE: Beneficial ownership and account control")
        recommendations.append("💰 TRACE: Source and destination of funds")
        
        return recommendations
    
    def _detect_classic_mule_pattern(self, df: pd.DataFrame, 
                                   account_id: str = None) -> Optional[MuleAccountAlert]:
        """
        ...
        try:
            ...
            if len(credits) < self.config['min_collection_transactions'] or len(debits) == 0:
                return None
            
            # Adaptive analysis of collection phase (credits)
            # Use statistical analysis instead of fixed thresholds
            ...
            # Define "small" credits as those below median (adaptive to account pattern)
            small_credits = credits[credits['CREDIT'] <= credit_median]
            
            collection_analysis = {
                'total_credits': len(credits),
                'small_credits': len(small_credits),
                'small_credit_ratio': len(small_credits) / len(credits) if len(credits) > 0 else 0,
                ...
                'credit_coefficient_variation': float(np.std(credit_amounts) / (np.mean(credit_amounts) + 1e-10)),
                'credit_frequency_per_day': len(credits) / max(1, (credits['DATE'].max() - credits['DATE'].min()).days),
                'collection_period_days': (credits['DATE'].max() - credits['DATE'].min()).days,
                'adaptive_small_threshold': float(credit_median)
            }
            
            # Adaptive analysis of disbursement phase (debits)
            ...
            # Define "large" debits as those above median (adaptive to account pattern)
            large_debits = debits[debits['DEBIT'] >= debit_median]
            
            disbursement_analysis = {
                'total_debits': len(debits),
                'large_debits': len(large_debits),
                'large_debit_ratio': len(large_debits) / len(debits) if len(debits) > 0 else 0,
                ...
                'debit_coefficient_variation': float(np.std(debit_amounts) / (np.mean(debit_amounts) + 1e-10)),
                'debit_frequency_per_day': len(debits) / max(1, (debits['DATE'].max() - debits['DATE'].min()).days),
                'adaptive_large_threshold': float(debit_median)
            }
            
            # Calculate adaptive mule pattern indicators
            confidence_score = 0.0
            risk_indicators = []
            
            # Indicator 1: Asymmetric transaction patterns (25% weight)
            # Many small credits vs few large debits (relative to account's own pattern)
            ...
            if credit_debit_count_ratio >= self.config['collection_disbursement_ratio']:
                ...
                risk_indicators.append(f"Asymmetric pattern: {credit_debit_count_ratio:.1f}x more credits than debits")
            
            # Indicator 2: Amount size disparity (25% weight)
            # Large debits relative to small credits (adaptive to account pattern)
            if collection_analysis['median_credit_amount'] > 0:
                debit_credit_ratio = disbursement_analysis['median_debit_amount'] / collection_analysis['median_credit_amount']
                if debit_credit_ratio >= self.config['min_disbursement_amount_ratio']:
                    score_weight = min(0.25, (debit_credit_ratio - 3) * 0.05)
                    confidence_score += score_weight
                    risk_indicators.append(f"Median debit is {debit_credit_ratio:.1f}x larger than median credit")
            
            # Indicator 3: Collection concentration pattern (20% weight)
            # High concentration of small amounts in credits
            if collection_analysis['small_credit_ratio'] > 0.6:  # >60% below median
                score_weight = (collection_analysis['small_credit_ratio'] - 0.6) * 0.5
                confidence_score += score_weight
                risk_indicators.append(f"{collection_analysis['small_credit_ratio']*100:.1f}% of credits are below median amount")
            
            # Indicator 4: Disbursement concentration pattern (15% weight)
            # Concentration of large amounts in debits
            if disbursement_analysis['large_debit_ratio'] > 0.4:  # >40% above median
                score_weight = (disbursement_analysis['large_debit_ratio'] - 0.4) * 0.25
                confidence_score += score_weight
                risk_indicators.append(f"{disbursement_analysis['large_debit_ratio']*100:.1f}% of debits are above median amount")
            
            # Indicator 5: Timing patterns (15% weight)
            timing_score = self._analyze_mule_timing_patterns(credits, debits)
            confidence_score += timing_score * 0.15
            if timing_score > 0.3:
                risk_indicators.append("Suspicious timing patterns detected between collections and disbursements")
            
            # Apply sensitivity multiplier and check threshold
            sensitivity_multiplier = self.config.get('sensitivity_multiplier', 1.0)
            adjusted_confidence = confidence_score * sensitivity_multiplier
            
            # Dynamic threshold based on sensitivity
            confidence_threshold = 0.4 / sensitivity_multiplier  # Lower threshold for higher sensitivity
            
            if adjusted_confidence >= confidence_threshold:
                return MuleAccountAlert(
                    account_id=account_id or "Unknown",
                    confidence_score=min(1.0, adjusted_confidence),  # Use adjusted confidence
                    ...
                    risk_indicators=risk_indicators,
                    recommended_actions=self._generate_mule_recommendations(confidence_score, risk_indicators)
                )
    ...
    def _detect_periodic_mule_pattern(self, df: pd.DataFrame, 
                                    account_id: str = None) -> Optional[MuleAccountAlert]:
        """
        ...
        try:
            ...
            if len(debits) < 3:  # Need at least 3 debits to detect periodicity
                return None
            ...
            # Calculate confidence based on periodicity strength
            confidence_score = 0.0
            risk_indicators = []
            ...
            if periodicity_analysis['periodicity_strength'] > 0.7:
                ...
                risk_indicators.append(f"Strong {periodicity_analysis['detected_period']} disbursement pattern")
            ...
            if confidence_score >= 0.5:  # 50% confidence threshold for periodic patterns
                return MuleAccountAlert(
                    account_id=account_id or "Unknown",
    ...
    def _detect_threshold_mule_pattern(self, df: pd.DataFrame, 
                                     account_id: str = None) -> Optional[MuleAccountAlert]:
        """
        ...
        try:
            ...
            thresholds = [10000, 20000, 50000, 100000]  # CTR and internal thresholds
            ...
            for threshold in thresholds:
                # Check transactions just below threshold
                near_threshold = df[
                    (df['amount'] >= threshold * 0.85) & 
                    (df['amount'] < threshold)
                ]
                
                if len(near_threshold) > 0:
                    threshold_ratio = len(near_threshold) / len(df)
                    if threshold_ratio > 0.2:  # >20% near threshold
                        total_threshold_score += threshold_ratio
                        risk_indicators.append(f"{len(near_threshold)} transactions just below ₹{threshold:,} threshold")
            
            # Check for structured amounts (round numbers)
            round_amounts = df[df['amount'] % 1000 == 0]
            round_ratio = len(round_amounts) / len(df) if len(df) > 0 else 0
            
            if round_ratio > 0.6:  # >60% round numbers
                total_threshold_score += 0.3
                risk_indicators.append(f"{round_ratio*100:.1f}% of transactions use round numbers")
            
            confidence_score = min(1.0, total_threshold_score)
            
            if confidence_score >= 0.4:  # 40% confidence threshold
                return MuleAccountAlert(
                    account_id=account_id or "Unknown",
                    confidence_score=confidence_score,
    ...
    def _analyze_mule_timing_patterns(self, credits: pd.DataFrame, debits: pd.DataFrame) -> float:
        ...
        try:
            ...
            # Pattern 1: Credits cluster before debits
            for _, debit in debits.iterrows():
                debit_date = debit['DATE']
                # Look for credits in the 7 days before this debit
                recent_credits = credits[
                    (credits['DATE'] >= debit_date - timedelta(days=7)) &
                    (credits['DATE'] < debit_date)
                ]
                
                if len(recent_credits) >= 3:  # 3+ credits before debit
                    timing_score += 0.2
            
            # Pattern 2: Regular intervals between debits
            if len(debits) >= 3:
                ...
                if len(debit_intervals) > 0:
                    ...
                    # Low variance in intervals indicates regularity
    ...
    def _analyze_disbursement_periodicity(self, debits: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate intervals between debits
            intervals = debits['DATE'].diff().dt.days.dropna()
            
            if len(intervals) == 0:
                return {'is_periodic': False}
            
            # Check for common periods (weekly=7, bi-weekly=14, monthly=30)
            common_periods = [7, 14, 30]
            best_period = None
            best_score = 0.0
            
            for period in common_periods:
                # Count intervals close to this period
    ...
    def _generate_mule_recommendations(self, confidence_score: float, 
                                     risk_indicators: List[str]) -> List[str]:
        """Generate specific recommendations for mule account alerts"""
        recommendations = []
        
        if confidence_score >= 0.8:
            recommendations.append("🚨 IMMEDIATE INVESTIGATION REQUIRED - High confidence mule account pattern")
        ...
        # Specific recommendations based on risk indicators
    ...
    def create_mule_detection_summary(self, alerts: List[MuleAccountAlert]) -> Dict[str, Any]:
        ...
        pattern_counts = {}
        for alert in alerts:
            pattern_counts[alert.pattern_type] = pattern_counts.get(alert.pattern_type, 0) + 1
        
        return {
            'total_alerts': len(alerts),
            'highest_confidence': max(alert.confidence_score for alert in alerts),
            'pattern_types': pattern_counts,
            'high_confidence_alerts': len([a for a in alerts if a.confidence_score >= 0.7]),
            'summary': f"Detected {len(alerts)} potential mule account patterns"
        }
    
    def export_mule_alerts_to_dataframe(self, alerts: List[MuleAccountAlert]) -> pd.DataFrame:
        ...
        for alert in alerts:
            row = {
                ...
                'primary_risk_indicator': alert.risk_indicators[0] if alert.risk_indicators else 'None'
            }
            ...
            if alert.pattern_type == 'classic_mule':
                row.update({
                    ...
                    'small_credit_ratio': alert.collection_phase.get('small_credit_ratio', 0),
                    'large_debit_ratio': alert.disbursement_phase.get('large_debit_ratio', 0)
                })
