...
class TimeBasedAnalytics:
    ...
    def analyze_transaction_trends(self, df: pd.DataFrame, 
                                 date_column: str = 'DATE',
                                 debit_column: str = 'DEBIT',
                                 credit_column: str = 'CREDIT',
                                 time_granularity: str = 'daily') -> Dict[str, Any]:
        """
        ...
        # Prepare data
        df_clean = self._prepare_time_series_data(df, date_column, debit_column, credit_column)
    ...
    def _prepare_time_series_data(self, df: pd.DataFrame, date_col: str, 
                                debit_col: str, credit_col: str) -> pd.DataFrame:
    ...
    def _aggregate_by_time(self, df: pd.DataFrame, granularity: str) -> pd.DataFrame:
        ...
        agg_data = df.groupby('time_key').agg({
            ...
            'day_of_week': lambda x: x.mode().iloc[0] if not x.empty else 'Monday'
        }).reset_index()
        ...
        agg_data['debit_credit_ratio'] = agg_data['DEBIT_sum'] / (agg_data['CREDIT_sum'] + 1e-10)
        agg_data['activity_intensity'] = agg_data['DEBIT_count'] + agg_data['CREDIT_count']
        agg_data['volatility'] = (agg_data['DEBIT_std'].fillna(0) + agg_data['CREDIT_std'].fillna(0)) / 2
    ...
    def _calculate_data_summary(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'date_range': {
                ...
                'span_days': (time_series['time_key'].max() - time_series['time_key'].min()).days
            },
            ...
            'most_active_period': time_series.loc[time_series['activity_intensity'].idxmax(), 'time_key'],
            'highest_debit_period': time_series.loc[time_series['DEBIT_sum'].idxmax(), 'time_key'],
            'highest_credit_period': time_series.loc[time_series['CREDIT_sum'].idxmax(), 'time_key']
        }
    ...
    def _analyze_trends(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in ['DEBIT_sum', 'CREDIT_sum', 'net_flow_sum', 'activity_intensity']:
            if metric in time_series.columns:
                trend_result = self._calculate_linear_trend(time_series['time_key'], time_series[metric])
        ...
        if window_size >= 2:
            results['moving_averages'] = {
                'debit_ma': time_series['DEBIT_sum'].rolling(window=window_size).mean().tolist(),
                'credit_ma': time_series['CREDIT_sum'].rolling(window=window_size).mean().tolist(),
                'net_flow_ma': time_series['net_flow_sum'].rolling(window=window_size).mean().tolist()
            }
        ...
        results['overall_assessment'] = {
            'debit_trend_direction': 'increasing' if debit_trend.get('slope', 0) > 0 else 'decreasing',
            'credit_trend_direction': 'increasing' if credit_trend.get('slope', 0) > 0 else 'decreasing',
            'debit_trend_strength': abs(debit_trend.get('r_squared', 0)),
            'credit_trend_strength': abs(credit_trend.get('r_squared', 0)),
            'trends_aligned': (debit_trend.get('slope', 0) * credit_trend.get('slope', 0)) > 0
        }
        
        return results
    
    def _calculate_linear_trend(self, x_values: pd.Series, y_values: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            n = len(x_clean)
            t_stat = slope * np.sqrt((n - 2) / (1 - r_squared + 1e-10)) * np.sqrt(np.sum((x_clean - np.mean(x_clean)) ** 2))
            p_value = 2 * (1 - abs(t_stat) / (abs(t_stat) + n - 2))  # Simplified p-value approximation
    ...
    def _detect_seasonal_patterns(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Monthly patterns (if we have enough data spanning multiple months)
    ...
    def _analyze_day_of_week_patterns(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        """Analyze patterns by day of week"""
        # This is a simplified version - in practice you'd need the original transaction data
        # to properly analyze day-of-week patterns
        return {
    ...
    def _analyze_monthly_patterns(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            # Extract month from time_key
            time_series['month'] = pd.to_datetime(time_series['time_key']).dt.month
            
            monthly_stats = time_series.groupby('month').agg({
                'DEBIT_sum': ['mean', 'std'],
                'CREDIT_sum': ['mean', 'std'],
            ...
            return {
                ...
                'seasonal_variation': {
                    'debit_cv': float(monthly_stats['DEBIT_sum_std'].mean() / (monthly_stats['DEBIT_sum_mean'].mean() + 1e-10)),
                    'credit_cv': float(monthly_stats['CREDIT_sum_std'].mean() / (monthly_stats['CREDIT_sum_mean'].mean() + 1e-10))
                }
            }
        except Exception:
            return {'pattern_detected': False, 'error': 'Insufficient data for monthly analysis'}
    ...
    def _detect_recurring_cycles(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for cycle_length in [7, 14, 30]:  # Weekly, bi-weekly, monthly cycles
                if len(debit_series) >= cycle_length * 2:
                    debit_autocorr = self._calculate_autocorrelation(debit_series, cycle_length)
                    credit_autocorr = self._calculate_autocorrelation(credit_series, cycle_length)
                    ...
                    if debit_autocorr > 0.3 or credit_autocorr > 0.3:
                        cycles_detected.append({
                            ...
                            'strength': 'strong' if max(debit_autocorr, credit_autocorr) > 0.6 else 'moderate'
                        })
    ...
    def _analyze_transaction_velocity(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        """Analyze transaction velocity and frequency patterns"""
        velocity_metrics = {}
        
        # Calculate velocity metrics
        velocity_metrics['average_transactions_per_period'] = float(time_series['activity_intensity'].mean())
        velocity_metrics['max_transactions_per_period'] = int(time_series['activity_intensity'].max())
        velocity_metrics['velocity_volatility'] = float(time_series['activity_intensity'].std())
        
        # Identify high-velocity periods
        velocity_threshold = time_series['activity_intensity'].quantile(0.8)
        high_velocity_periods = time_series[time_series['activity_intensity'] > velocity_threshold]
        ...
        velocity_metrics['average_acceleration'] = float(time_series['velocity_change'].mean())
        velocity_metrics['max_acceleration'] = float(time_series['velocity_change'].max())
        velocity_metrics['max_deceleration'] = float(time_series['velocity_change'].min())
    ...
    def _detect_time_based_anomalies(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in ['DEBIT_sum', 'CREDIT_sum', 'activity_intensity']:
            if metric in time_series.columns:
                ...
                anomalous_periods = time_series[
                    (time_series[metric] < lower_bound) | (time_series[metric] > upper_bound)
                ]
                ...
                for _, period in anomalous_periods.iterrows():
                    anomalies['statistical_anomalies'].append({
                        ...
                        'severity': 'high' if period[metric] > upper_bound + iqr else 'moderate'
                    })
        
        # Velocity anomalies (sudden spikes in transaction frequency)
        ...
        velocity_anomalies = time_series[time_series['activity_intensity'] > velocity_threshold]
        for _, period in velocity_anomalies.iterrows():
            anomalies['velocity_anomalies'].append({
                ...
                'severity': 'high' if period['activity_intensity'] > velocity_mean + 3 * velocity_std else 'moderate'
            })
        
        return anomalies
    
    def _analyze_debit_credit_correlation(self, time_series: pd.DataFrame) -> Dict[str, float]:
        """Analyze correlation between debit and credit patterns"""
        try:
            correlation_matrix = time_series[['DEBIT_sum', 'CREDIT_sum', 'net_flow_sum', 'activity_intensity']].corr()
            
            return {
                'debit_credit_correlation': float(correlation_matrix.loc['DEBIT_sum', 'CREDIT_sum']),
                'debit_activity_correlation': float(correlation_matrix.loc['DEBIT_sum', 'activity_intensity']),
                'credit_activity_correlation': float(correlation_matrix.loc['CREDIT_sum', 'activity_intensity']),
                'net_flow_activity_correlation': float(correlation_matrix.loc['net_flow_sum', 'activity_intensity'])
            }
        except Exception:
            return {
    ...
    def _detect_cyclical_patterns(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in ['DEBIT_sum', 'CREDIT_sum']:
            if metric in time_series.columns and len(time_series) > 10:
                ...
                # Calculate moving average to detect cycles
                ...
                if window >= 2:
                    ...
                    # Count zero crossings as a simple cycle indicator
                    zero_crossings = np.sum(np.diff(np.sign(detrended)) != 0)
                    cycle_frequency = zero_crossings / len(series_values) if len(series_values) > 0 else 0
                    
                    cyclical_analysis[metric] = {
                        'cycle_frequency': float(cycle_frequency),
                        'has_cycles': cycle_frequency > 0.1,
                        'cycle_strength': float(np.std(detrended) / (np.mean(np.abs(series_values)) + 1e-10))
                    }
        ...
        for metric in ['DEBIT_sum', 'CREDIT_sum', 'net_flow_sum']:
            if metric in time_series.columns:
                ...
                volatility_analysis[metric] = {
                    ...
                    'coefficient_of_variation': float(values.std() / (values.mean() + 1e-10)),
    ...
    def _calculate_volatility_trend(self, series: pd.Series) -> str:
        ...
        try:
            ...
            # Trend in volatility
            vol_trend = self._calculate_linear_trend(pd.Series(range(len(rolling_vol))), rolling_vol)
    ...
    def _analyze_cash_flow_patterns(self, time_series: pd.DataFrame) -> Dict[str, Any]:
        ...
        flow_analysis['flow_balance'] = {
            ...
            'neutral_periods': len(time_series) - positive_flow_periods - negative_flow_periods,
            'net_positive_ratio': positive_flow_periods / len(time_series) if len(time_series) > 0 else 0
        }
    ...
    def create_trend_dashboard(self, analysis_results: Dict[str, Any]) -> go.Figure:
        ...
        if 'visualizations' not in analysis_results or not analysis_results['visualizations']:
            return go.Figure().add_annotation(text="No data available for visualization", 
        ...
        fig = make_subplots(
            rows=3, cols=2,
            subplot_titles=('Debit vs Credit Trends', 'Net Flow Analysis', 
                          'Transaction Activity', 'Volatility Analysis',
                          'Cumulative Flow', 'Correlation Heatmap'),
        ...
        # 1. Debit vs Credit Trends
        fig.add_trace(go.Scatter(x=dates, y=debits, name='Debits', line=dict(color='red')), row=1, col=1)
        fig.add_trace(go.Scatter(x=dates, y=credits, name='Credits', line=dict(color='green')), row=1, col=1)
        
        # Add moving averages if available
        if 'moving_averages' in viz_data:
            ma_data = viz_data['moving_averages']
            fig.add_trace(go.Scatter(x=dates, y=ma_data['debit_ma'], name='Debit MA', 
                                   line=dict(color='red', dash='dash')), row=1, col=1)
            fig.add_trace(go.Scatter(x=dates, y=ma_data['credit_ma'], name='Credit MA', 
                                   line=dict(color='green', dash='dash')), row=1, col=1)
        
        # 2. Net Flow Analysis
        colors = ['red' if x < 0 else 'green' for x in net_flow]
        fig.add_trace(go.Bar(x=dates, y=net_flow, name='Net Flow', marker_color=colors), row=1, col=2)
        
        # 3. Transaction Activity
        fig.add_trace(go.Scatter(x=dates, y=activity, name='Activity', line=dict(color='blue')), row=2, col=1)
        
        # 4. Volatility (using standard deviation of recent periods)
        if len(debits) > 5:
            volatility = pd.Series(debits).rolling(window=5).std().fillna(0).tolist()
            fig.add_trace(go.Scatter(x=dates, y=volatility, name='Debit Volatility', 
                                   line=dict(color='orange')), row=2, col=2)
        
        # 5. Cumulative Flow
        cumulative_net = np.cumsum(net_flow).tolist()
        fig.add_trace(go.Scatter(x=dates, y=cumulative_net, name='Cumulative Net Flow', 
                               line=dict(color='purple')), row=3, col=1)
        
        # 6. Simple correlation visualization (if we have correlation data)
        if 'correlation_analysis' in analysis_results:
            corr_data = analysis_results['correlation_analysis']
            corr_values = list(corr_data.values())
            corr_labels = list(corr_data.keys())
            
            fig.add_trace(go.Bar(x=corr_labels, y=corr_values, name='Correlations'), row=3, col=2)
        
        # Update layout
        fig.update_layout(
            title="Transaction Trend Analysis Dashboard",
            height=900,
            showlegend=True
        )
    ...
    def generate_trend_insights(self, analysis_results: Dict[str, Any]) -> List[str]:
        ...
        if 'data_summary' in analysis_results:
            ...
            insights.append(f"📊 Analyzed {summary.get('total_periods', 0)} time periods")
            insights.append(f"💰 Total debits: ₹{summary.get('total_debits', 0):,.2f}")
            insights.append(f"💰 Total credits: ₹{summary.get('total_credits', 0):,.2f}")
        ...
        if 'trend_analysis' in analysis_results:
            ...
            if trends:
                insights.append(f"📈 Debit trend: {trends.get('debit_trend_direction', 'unknown')}")
                insights.append(f"📈 Credit trend: {trends.get('credit_trend_direction', 'unknown')}")
        ...
        if 'velocity_analysis' in analysis_results:
            ...
            insights.append(f"⚡ Average transaction velocity: {avg_velocity:.1f} transactions per period")
