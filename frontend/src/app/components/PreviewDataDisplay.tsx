...
export default function PreviewDataDisplay({
  file,
  bankPreset,
  columnMapping,
  csvValidation,
}: PreviewDataDisplayProps) {
  ...
  useEffect(() => {
    const fetchPreviewData = async () => {
      ...
      try {
        console.log(
          "Fetching preview data for file:",
          file.name,
          "with bank:",
          bankPreset,
          "and mapping:",
  ...
  const calculateExtractionStats = () => {
    ...
    // Count successful extractions (where counterparty was extracted, not null/empty, and is different from description)
    const successfulExtractions = previewData.transactions.filter((tx) => {
      console.log("Evaluating transaction:", tx.counterparty_merged);
      return (
    ...
    const failedExtractions =
      previewData.transactions.length - successfulExtractions;

    const total = previewData.transactions.length;
    const successRate =
      total > 0 ? Math.round((successfulExtractions / total) * 100) : 0;
    const failureRate =
      total > 0 ? Math.round((failedExtractions / total) * 100) : 0;
  ...
  if (
    !previewData ||
    (previewData.transactions.length === 0 && previewData.errors.length === 0)
  ) {
    return null;
  }
    ...
    <div className="mt-4 border border-gray-200 rounded-md p-4 bg-gray-50">
        ...
        <div className="mb-4 p-3 bg-white rounded-md border border-gray-200">
          <div className="flex items-center justify-between">
            <div className="text-xs font-medium text-gray-700">
              Extraction Stats for{" "}
              <span className="font-semibold">{bankPreset}</span> preset
            </div>
          ...
          <div className="mt-1 text-xs text-gray-500">
            {stats.successfulExtractions} successful, {stats.failedExtractions}{" "}
            failed extractions
          </div>
        </div>
      )}
      ...
      {previewData.transactions.length > 0 ? (
              ...
              {previewData.transactions
                ...
                .map((transaction, index) => (
                  <tr key={index} className="text-xs">
                    ...
                    <td
                      className="px-3 py-2 max-w-xs truncate"
                      title={transaction.counterparty_merged}
                    >
        ...
        <div className="text-sm text-gray-500">
          No transactions could be extracted with the current bank preset and
          column mapping.
        </div>
      )}
    </div>
