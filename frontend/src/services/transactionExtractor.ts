...
type PdfTableCacheEntry = {
  headers: string[];
  rows: string[][];
};

const pdfTableCache = new WeakMap<File, PdfTableCacheEntry>();
...
async function getPdfHeadersAndRows(
  file: File,
  timeoutMs?: number
): Promise<PdfTableCacheEntry> {
  const cached = pdfTableCache.get(file);
  if (cached) return cached;

  const json = await fetchExtractTablesJson(file, timeoutMs);
  const tables = extractTablesFromResponse(json);
  const derived = deriveHeadersAndRows(tables);
  pdfTableCache.set(file, derived);
  return derived;
}
...
export const transactionExtractorService = {
  ...
  async validatePDFColumns(file: File): Promise<CSVValidationResult> {
    const { headers } = await getPdfHeadersAndRows(file);
    return validateCSVColumns(headers);
  },
  ...
  async extractFromPDF(
    ...
    try {
      const { headers, rows: dataRows } = await getPdfHeadersAndRows(
        file,
        120000
      );
