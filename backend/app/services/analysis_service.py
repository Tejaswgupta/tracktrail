...
import pandas as pd
import polars as pl
from app.core.exceptions import (AnalysisError, EntityNotFoundError,
                                 ValidationError)
...
class AnalysisService:
    ...
    async def _fetch_and_prepare_data(
        ...
        try:
            ...
            if (convert_to_polars and len(df) == 0) or (not convert_to_polars and df.empty):
    ...
    def _perform_cash_flow_analysis(self, df: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            cash_keywords = kwargs.get('cash_keywords', ['CASH', 'ATM', 'WITHDRAWAL', 'CHQ'])
            ...
            monthly_freq_dict = {str(k): int(v) for k, v in monthly_freq.to_dict().items()}
            ...
            cash_amounts = pd.concat([
                cash_txns['DEBIT'].dropna(),
                cash_txns['CREDIT'].dropna()
            ])
            ...
            results = {
                ...
                "frequency_analysis": {
                    ...
                    "peak_activity_day": dow_freq.idxmax() if not dow_freq.empty else None
                },
                "amount_patterns": {
                    "average_amount": float(cash_amounts.mean()) if not cash_amounts.empty else 0,
                    "median_amount": float(cash_amounts.median()) if not cash_amounts.empty else 0,
                    "max_amount": float(cash_amounts.max()) if not cash_amounts.empty else 0,
                    "min_amount": float(cash_amounts.min()) if not cash_amounts.empty else 0
                },
                "temporal_patterns": {
                    "date_range_days": (cash_txns['DATE'].max() - cash_txns['DATE'].min()).days,
                    "analysis_period": {
    ...
    async def analyze_counterparty_trends(
        ...
        **kwargs
        ...
        """
        try:
            logger.info(f"Starting counterparty trend analysis for {len(entity_ids)} entities")
            ...
            counterparty_results = self.counterparty_analyzer.analyze_counterparty_trends(
                df=df,
            ...
            # Generate insights
            insights = self.counterparty_analyzer.generate_counterparty_insights(counterparty_results)
            
            # Get high-risk counterparties
            high_risk_threshold = kwargs.get('risk_threshold', 0.6)
            high_risk_counterparties = self.counterparty_analyzer.get_high_risk_counterparties(
            ...
            return self._format_analysis_result(
                ...
                results={
                    ...
                    "summary": {
                        ...
                        "average_risk_score": sum(r.risk_score for r in counterparty_results.values()) / len(counterparty_results) if counterparty_results else 0
                    }
                },
                transaction_count=len(df),
                date_range=self._get_date_range(df)
    ...
    async def detect_mule_accounts(
        ...
        **kwargs
        ...
        """
        try:
            logger.info(f"Starting mule account detection for {len(entity_ids)} entities")
            ...
            account_identifier = kwargs.get('account_identifier', entity_ids[0] if entity_ids else None)
            ...
            if sensitivity_multiplier != 1.0:
                self.mule_detector.config['sensitivity_multiplier'] = sensitivity_multiplier
            ...
            return self._format_analysis_result(
                ...
                results={
                    ...
                    "highest_confidence_alert": max(mule_alerts, key=lambda x: x.confidence_score) if mule_alerts else None,
                    "summary": {
                        "entities_analyzed": len(entity_ids),
                        "alerts_generated": len(mule_alerts),
                        "high_confidence_alerts": len([a for a in mule_alerts if a.confidence_score > 0.7]),
                        "pattern_types_detected": list(set(a.pattern_type for a in mule_alerts))
                    }
                },
                transaction_count=len(df),
                date_range=self._get_date_range(df)
    ...
    async def detect_cycles(
        ...
        **kwargs
        ...
        try:
            ...
            if len(entity_ids) == 1:
                return await self._detect_simple_round_trips(entity_ids, date_from, date_to, **kwargs)
            else:
                return await self._detect_network_cycles(entity_ids, date_from, date_to, **kwargs)
    ...
    def _find_round_trips(self, df: pl.DataFrame, tolerance: float, days: int, min_amount: float) -> Dict[str, Any]:
        """Extract round trip detection logic from round_trip.py"""
        try:
            results = []
            ...
            if "counterparty" not in df.columns:
                return {
                    ...
                    "message": "Counterparty information required for round trip detection",
                    "round_trips": []
                }
            
            counterparties = [c for c in df["counterparty"].drop_nulls().unique().to_list() if c]
            
            for cp in counterparties:
                cp_df = df.filter(pl.col("counterparty") == cp)
                            ...
                            if days_diff <= days:
                                ...
                                if amount_diff <= tolerance:
                                    results.append({
                                        ...
                                        "outgoing_date": debit["DATE"].strftime('%Y-%m-%d'),
                                        "incoming_amount": float(credit["CREDIT"]),
                                        "incoming_date": credit["DATE"].strftime('%Y-%m-%d'),
                                        "days_gap": days_diff,
                                        "amount_difference_percent": round(amount_diff, 1)
                                    })
            ...
            logger.error(f"Round trip computation failed: {str(e)}")
            raise AnalysisError(f"Round trip computation failed: {str(e)}")
    ...
    async def _detect_network_cycles(
        ...
        **kwargs
        ...
        try:
            ...
            centrality_metrics = self.network_detector.calculate_centrality_metrics(graph)
            
            # Identify hub entities
            hub_entities = self.network_detector.identify_hub_entities(graph, centrality_metrics=centrality_metrics)
            ...
            for cycle in detected_cycles:
                formatted_cycles.append({
                    ...
                    "first_transaction_date": cycle.first_transaction_date.isoformat() if cycle.first_transaction_date else None,
                    "last_transaction_date": cycle.last_transaction_date.isoformat() if cycle.last_transaction_date else None,
                    "transactions": cycle.transactions
                })
            
            # Calculate high confidence cycles
            high_confidence_cycles = [c for c in detected_cycles if c.confidence_score > 0.7]
            ...
            return self._format_analysis_result(
                ...
                results={
                    ...
                    "cycles_count": len(detected_cycles),  # Keep for backward compatibility
    ...
    def _build_transaction_graph(self, df: pd.DataFrame) -> 'nx.DiGraph':
        ...
        try:
            ...
            for entity_id, entity_df in entity_groups:
                entity_name = entity_df.iloc[0].get('entity_name', f'Entity_{entity_id}')
                    ...
                    if counterparty and counterparty != '':
                        ...
                        if counterparty_entity_id:
                            ...
                            if not graph.has_node(target_node_id):
                                ...
                                counterparty_data = df[df['entity_id'] == counterparty_entity_id].iloc[0]
                                graph.add_node(target_node_id,
                                             name=counterparty_data.get('entity_name', counterparty),
                                             entity_type=counterparty_data.get('entity_type', 'Unknown'),
                                             node_type='entity',
                                             total_debits=0,
                                             total_credits=0,
                                             transaction_count=0)
                        ...
                        if graph.has_edge(source, target):
                            ...
                            graph[source][target]['transactions'].append({
                                'amount': amount,
                                'date': pd.to_datetime(row['DATE']) if pd.notna(row['DATE']) else datetime.now(),
                                'description': row.get('DESCRIPTION', ''),
                                'transaction_id': row.get('transaction_id', ''),
                                'transaction_type': 'debit' if is_debit else 'credit',
                                'counterparty': counterparty,
                                'counterparty_resolved': counterparty_entity_id is not None
                            })
                            ...
                            graph.add_edge(source, target, 
                                         ...
                                         total_amount=amount,
                                         transaction_count=1,
                                         edge_type='internal' if counterparty_entity_id else 'external')
            
            # Count resolved vs external relationships
            entity_nodes = [n for n, d in graph.nodes(data=True) if d['node_type'] == 'entity']
            external_nodes = [n for n, d in graph.nodes(data=True) if d['node_type'] == 'external']
            internal_edges = [e for e in graph.edges(data=True) if e[2].get('edge_type') == 'internal']
            
            logger.info(f"Built transaction graph with {graph.number_of_nodes()} nodes ({len(entity_nodes)} entities, {len(external_nodes)} external) and {graph.number_of_edges()} edges ({len(internal_edges)} internal)")
            return graph
    ...
    def _create_entity_name_mapping(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        try:
            ...
            logger.debug(f"Created entity name mapping with {len(entity_mapping)} entries")
    ...
    def _normalize_entity_name(self, name: str) -> str:
        ...
        suffixes_to_remove = [
            ...
            ' llp', ' llc', ' corp', ' corporation', ' co', ' company'
        ]
    ...
    def _fuzzy_match_entity(
        ...
        try:
            ...
            if best_match:
                logger.debug(f"Fuzzy matched '{counterparty}' to entity {best_match} (score: {best_score:.3f})")
    ...
    def _analyze_entity_relationships(self, graph: 'nx.DiGraph') -> Dict[str, Any]:
        ...
        try:
            ...
            internal_flow = sum(edge[2]['total_amount'] for edge in entity_to_entity_edges)
            outgoing_flow = sum(edge[2]['total_amount'] for edge in entity_to_external_edges)
            incoming_flow = sum(edge[2]['total_amount'] for edge in external_to_entity_edges)
            ...
            total_counterparty_edges = len(entity_to_external_edges) + len(external_to_entity_edges) + len(entity_to_entity_edges)
            resolved_edges = len(entity_to_entity_edges)
            resolution_rate = (resolved_edges / total_counterparty_edges * 100) if total_counterparty_edges > 0 else 0
            ...
            return {
                ...
                "network_density": round(
                    len(entity_to_entity_edges) / (len(entity_nodes) * (len(entity_nodes) - 1)) * 100, 2
                ) if len(entity_nodes) > 1 else 0
            }
    ...
    async def analyze_rapid_movements(
        ...
        **kwargs
        ...
        """
        try:
            logger.info(f"Starting rapid movement analysis for {len(entity_ids)} entities")
            ...
            rapid_patterns = self._detect_rapid_movements(df, hours, tolerance, min_amount)
    ...
    def _detect_rapid_movements(self, df: pd.DataFrame, hours: int, tolerance: float, min_amount: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if rapid_patterns:
                ...
                if {"in_counterparty", "out_counterparty"}.issubset(patterns_df.columns):
                    pair_counts = patterns_df.groupby(["in_counterparty", "out_counterparty"]).size().reset_index(name="occurrences")
                    repeated_pairs = pair_counts[pair_counts["occurrences"] >= 2].sort_values("occurrences", ascending=False).to_dict('records')
    ...
    async def analyze_transfer_patterns(
        ...
        **kwargs
        ...
        """
        try:
            logger.info(f"Starting transfer pattern analysis for {len(entity_ids)} entities")
            ...
            min_amount = kwargs.get('min_amount', 1000)  # Minimum amount
            min_occurrences = kwargs.get('min_occurrences', 2)  # Minimum pattern occurrences
    ...
    def _find_transfer_patterns(self, df: pd.DataFrame, time_window: int, percentage_match: float, 
                               deviance: float, min_amount: float, min_occurrences: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, credit_txn in credits.iterrows():
                ...
                if len(candidate_debits) > 0:
                    ...
                    candidate_debits["amount_diff"] = abs(candidate_debits["DEBIT"] - credit_amount)
                    candidate_debits["date_diff"] = (candidate_debits["DATE"] - credit_date).dt.days
                    ...
                    potential_patterns.append({
                        ...
                        "percentage_transferred": (best_match["DEBIT"] / credit_txn["CREDIT"]) * 100
                    })
            ...
            for (source, destination), group in pattern_groups:
                if len(group) >= min_occurrences:
                    summary = {
                        ...
                        "average_percentage_transferred": float(group["percentage_transferred"].mean()),
    ...
    def _empty_analysis_result(self, analysis_type: str, entity_ids: List[str]) -> Dict[str, Any]:
    ...
    def _get_date_range_polars(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "start": start_date.strftime('%Y-%m-%dT%H:%M:%S') if start_date else None,
                "end": end_date.strftime('%Y-%m-%dT%H:%M:%S') if end_date else None
            }
        except Exception:
            return {"start": None, "end": None}
    
    def _generate_insights(self, analysis_type: str, results: Dict[str, Any]) -> List[str]:
        ...
        try:
            if analysis_type == "cash_flow":
                if results.get("cash_transactions_found"):
                    insights.append(f"Found {results.get('total_cash_transactions', 0)} cash transactions")
                    if results.get("large_cash_transactions", 0) > 0:
                        insights.append(f"⚠️ {results['large_cash_transactions']} large cash transactions detected")
                else:
                    insights.append("No cash transactions detected with specified keywords")
            
            elif analysis_type == "counterparty_trends":
                summary = results.get("summary", {})
                if summary.get("high_risk_count", 0) > 0:
                    insights.append(f"🚨 {summary['high_risk_count']} high-risk counterparties identified")
                insights.append(f"Analyzed {summary.get('total_counterparties_analyzed', 0)} counterparties")
            
            elif analysis_type == "mule_accounts":
                summary = results.get("summary", {})
                if summary.get("alerts_generated", 0) > 0:
                    insights.append(f"🚨 {summary['alerts_generated']} mule account alerts generated")
                    if summary.get("high_confidence_alerts", 0) > 0:
                        insights.append(f"⚠️ {summary['high_confidence_alerts']} high-confidence alerts")
            
            elif analysis_type in ["round_trips", "network_cycles"]:
                if analysis_type == "round_trips":
                    if results.get("round_trips_found"):
                        insights.append(f"Found {results.get('total_round_trips', 0)} round trip patterns")
                elif analysis_type == "network_cycles":
                    if results.get("cycles_count", 0) > 0:
                        insights.append(f"Detected {results['cycles_count']} network cycles")
            
            elif analysis_type == "rapid_movements":
                if results.get("rapid_movements_found"):
                    insights.append(f"Found {results.get('total_rapid_movements', 0)} rapid money movements")
                    if results.get("repeated_pairs"):
                        insights.append(f"⚠️ {len(results['repeated_pairs'])} repeated party pairs detected")
            
            elif analysis_type == "transfer_patterns":
                if results.get("transfer_patterns_found"):
                    insights.append(f"Found {results.get('total_patterns', 0)} repeated transfer patterns")
    ...
    def _extract_risk_indicators(self, analysis_type: str, results: Dict[str, Any]) -> List[str]:
        ...
        try:
            if analysis_type == "cash_flow":
                ...
                for cp in high_risk[:3]:  # Top 3 high-risk counterparties
                    risk_indicators.append(f"High-risk counterparty: {cp.get('name', 'Unknown')} (score: {cp.get('risk_score', 0):.2f})")
            
            elif analysis_type == "mule_accounts":
                alerts = results.get("mule_alerts", [])
                for alert in alerts[:3]:  # Top 3 alerts
                    risk_indicators.append(f"Mule account pattern: {alert.get('pattern_type', 'Unknown')} (confidence: {alert.get('confidence_score', 0):.2f})")
            
            elif analysis_type in ["round_trips", "network_cycles"]:
                if analysis_type == "round_trips" and results.get("round_trips_found"):
                    risk_indicators.append("Round trip transactions detected")
                elif analysis_type == "network_cycles" and results.get("cycles_count", 0) > 0:
