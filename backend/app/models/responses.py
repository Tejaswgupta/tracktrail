...
class AnalysisResponse(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(None, description="Request and processing metadata")
...
class AnalysisMetadata(BaseModel):
    ...
    date_range: Optional[Dict[str, datetime]] = Field(None, description="Analysis date range")
    processing_time_ms: Optional[int] = Field(None, description="Processing time in milliseconds")
    request_id: Optional[str] = Field(None, description="Unique request identifier")
    parameters: Optional[Dict[str, Any]] = Field(None, description="Analysis parameters used")
...
class CashFlowAnalysisData(BaseModel):
    ...
    transaction_patterns: List[Dict[str, Any]] = Field(..., description="Identified transaction patterns")
    time_series_data: List[Dict[str, Any]] = Field(..., description="Time-based flow data")
    risk_indicators: List[str] = Field(default_factory=list, description="Risk flags identified")


class CounterpartyTrendsData(BaseModel):
    """Specific data structure for counterparty trends analysis results."""
    
    top_counterparties: List[Dict[str, Any]] = Field(..., description="Most active counterparties")
    trend_analysis: Dict[str, Any] = Field(..., description="Trend analysis results")
    relationship_strength: Dict[str, float] = Field(..., description="Counterparty relationship strengths")
    anomalous_relationships: List[Dict[str, Any]] = Field(..., description="Unusual counterparty patterns")


class MuleAccountData(BaseModel):
    """Specific data structure for mule account detection results."""
    
    risk_score: float = Field(..., ge=0, le=1, description="Overall mule account risk score")
...
class CycleDetectionData(BaseModel):
    ...
    cycles_found: List[Dict[str, Any]] = Field(..., description="Detected transaction cycles")
    cycle_statistics: Dict[str, Any] = Field(..., description="Cycle analysis statistics")
    network_metrics: Dict[str, float] = Field(..., description="Network analysis metrics")
    suspicious_cycles: List[Dict[str, Any]] = Field(..., description="High-risk cycles identified")


class RapidMovementData(BaseModel):
    """Specific data structure for rapid movement analysis results."""
    
    rapid_sequences: List[Dict[str, Any]] = Field(..., description="Detected rapid movement sequences")
    velocity_metrics: Dict[str, float] = Field(..., description="Movement velocity statistics")
...
class TimeTrendsData(BaseModel):
    ...
    anomalies: List[Dict[str, Any]] = Field(..., description="Temporal anomalies detected")
    forecasting: Optional[Dict[str, Any]] = Field(None, description="Trend forecasting results")


class TransferPatternData(BaseModel):
    """Specific data structure for transfer pattern analysis results."""
    
    detected_patterns: List[Dict[str, Any]] = Field(..., description="Identified transfer patterns")
    network_analysis: Dict[str, Any] = Field(..., description="Network structure analysis")
    pattern_strength: Dict[str, float] = Field(..., description="Pattern strength scores")
    entity_roles: Dict[str, str] = Field(..., description="Entity roles in patterns")


# Union type for all possible analysis data types
AnalysisDataType = Union[
    ...
    TimeTrendsData,
    TransferPatternData,
    Dict[str, Any]  # Fallback for generic data
]