...
class CashFlowRequest(AnalysisRequest):
    ...
    include_patterns: Optional[bool] = Field(
        True,
        description="Include transaction patterns in analysis"
    )
    granularity: Optional[str] = Field(
        "daily",
        description="Time granularity for analysis (daily, weekly, monthly)"
    )
    cash_keywords: Optional[List[str]] = Field(
        None,
        description="List of keywords to filter cash flow transactions"
    )
...
class CounterpartyTrendsRequest(AnalysisRequest):
    ...
    min_transaction_count: Optional[int] = Field(
        5,
        ge=1,
        description="Minimum number of transactions to consider a counterparty"
    )
    trend_window_days: Optional[int] = Field(
        30,
        ge=1,
        le=365,
        description="Window size in days for trend analysis"
    )
...
class MuleAccountRequest(AnalysisRequest):
    ...
    velocity_threshold: Optional[float] = Field(
        10000.0,
        ge=0,
        description="Minimum transaction velocity to flag as suspicious"
    )
...
class RapidMovementRequest(AnalysisRequest):
    ...
    amount_threshold: Optional[float] = Field(
        5000.0,
        ge=0,
        description="Minimum amount threshold for rapid movement"
    )
    tolerance_percentage: Optional[float] = Field(
        5.0,
        ge=0,
        le=100,
        description="Tolerance percentage for amount matching"
    )
...
class TimeTrendsRequest(AnalysisRequest):
    ...
    aggregation_period: Optional[str] = Field(
        "daily",
        description="Time aggregation period (hourly, daily, weekly, monthly)"
    )
...
class TransferPatternRequest(AnalysisRequest):
    ...
    network_depth: Optional[int] = Field(
        ...
        ge=1,
        le=5,
        description="Maximum network depth for pattern analysis"
    )
    min_pattern_strength: Optional[float] = Field(
        0.7,
        ge=0.1,
        le=1.0,
        description="Minimum pattern strength threshold (0.1-1.0)"
    )
    
    @field_validator('pattern_types')
    @classmethod
    def validate_pattern_types(cls, v):
        allowed_patterns = ['layering', 'structuring', 'round_robin', 'fan_out', 'fan_in']
            ...
            "examples": {
                ...
                "comprehensive_analysis": {
                    ...
                    "value": {
                        ...
                        "pattern_types": ["layering", "structuring", "round_robin", "fan_out", "fan_in"],
                        "network_depth": 3,
