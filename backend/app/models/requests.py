...
class RegexCreateRequest(BaseModel):
    """Request payload when saving generated regex patterns."""

    workspace_id: str = Field(
        ..., description="Organization or workspace identifier"
    )
    name: str = Field(..., description="Label for the regex collection")
    patterns: List[str] = Field(
        ..., min_items=1, description="Array of regex strings generated by the tool"
    )
    description: Optional[str] = Field(
        None, description="Optional notes describing the intent"
    )
    source_csv: Optional[str] = Field(
        None, description="CSV filename or identifier used for generation"
    )
    created_by: Optional[str] = Field(
        None, description="User ID of the person saving the regex"
    )

    class Config:
        schema_extra = {
            "example": {
                "workspace_id": "dgddi-collection",
                "name": "NEFT Beneficiary Patterns",
                "patterns": ["^NEFT.*?-([A-Za-z\\s]+)-.*$"],
                "description": "Generic beneficiary capture for NEFT formats",
                "source_csv": "department-neft-sample.csv",
