...
@router.post(
    ...
    responses={
        ...
        404: {"description": "One or more entities not found", "model": ErrorResponse},
        500: {"description": "Analysis processing failed", "model": ErrorResponse},
    },
)
    ...
    try:
        ...
        if len(request.entity_ids) > 1:
            raise ValidationError(
                "AI LLM analysis currently supports single entity analysis only"
            )
        ...
        error_response = ErrorResponse(
            ...
            message=str(e),
            details={"entity_ids": request.entity_ids, "analysis_type": "ai_llm"},
        )
        return JSONResponse(
        ...
        error_response = ErrorResponse(
            ...
            details={"analysis_type": "ai_llm", "entity_ids": request.entity_ids},
        )
        return JSONResponse(
...
async def analyze_cash_flow(
    ...
    try:
        logger.info(
            f"Starting cash flow analysis for {len(request.entity_ids)} entities"
        )

        # Validate that this is single entity analysis (based on requirements)
        if len(request.entity_ids) > 1:
            raise ValidationError(
                "Cash flow analysis currently supports single entity analysis only"
            )

        # Extract analysis parameters from request
        analysis_params = {
            "cash_keywords": request.cash_keywords
            or ["CASH", "ATM", "WITHDRAWAL", "CHQ"],  # Default keywords
        ...
        logger.info(
            f"Cash flow analysis completed successfully in {processing_time_ms}ms"
        )
        ...
        error_response = ErrorResponse(
            error_code="ENTITY_NOT_FOUND",
            message=str(e),
            details={"entity_ids": request.entity_ids, "analysis_type": "cash_flow"},
        )
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content=error_response.model_dump(mode="json"),
        ...
        error_response = ErrorResponse(
            error_code="ANALYSIS_ERROR",
            message="Analysis processing failed. Please check your request and try again.",
            details={"analysis_type": "cash_flow", "entity_ids": request.entity_ids},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
        ...
        error_response = ErrorResponse(
            ...
            details={"analysis_type": "cash_flow"},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
...
async def analyze_counterparty_trends(
    ...
    try:
        logger.info(
            f"Starting counterparty trends analysis for {len(request.entity_ids)} entities"
        )

        # Validate that this is single entity analysis (based on requirements)
        if len(request.entity_ids) > 1:
            raise ValidationError(
                "Counterparty trends analysis currently supports single entity analysis only"
            )
        ...
        counterparty_count = (
            results.get("results", {})
            .get("summary", {})
            .get("total_counterparties_analyzed", 0)
        )
        high_risk_count = (
            results.get("results", {}).get("summary", {}).get("high_risk_count", 0)
        )
        ...
        logger.info(
            f"Counterparty trends analysis completed successfully in {processing_time_ms}ms"
        )
        ...
        error_response = ErrorResponse(
            error_code="INTERNAL_ERROR",
            message="An unexpected error occurred during analysis.",
            details={"analysis_type": "counterparty_trends"},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
...
async def detect_mule_accounts(
    ...
    try:
        logger.info(
            f"Starting mule account detection for {len(request.entity_ids)} entities"
        )

        # Validate that this is single entity analysis (based on requirements)
        if len(request.entity_ids) > 1:
            raise ValidationError(
                "Mule account detection currently supports single entity analysis only"
            )
        ...
        if alerts_count > 0:
            ...
            message = (
                "Mule account detection completed. No mule account patterns detected."
            )
        ...
        logger.info(
            f"Mule account detection completed successfully in {processing_time_ms}ms"
        )
        ...
        error_response = ErrorResponse(
            error_code="INTERNAL_ERROR",
            message="An unexpected error occurred during analysis.",
            details={"analysis_type": "mule_accounts"},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
    ...
    **Multiple Entities (Network Cycle Detection):**
    ...
    try:
        ...
        if entity_count == 1:
            ...
            round_trips_found = results.get("results", {}).get(
            ...
            multi_entity_cycles = results.get("results", {}).get(
                "multi_entity_cycles_found", 0
            )
            high_confidence_cycles = results.get("results", {}).get(
        ...
        error_response = ErrorResponse(
            ...
            details={
                "entity_ids": request.entity_ids,
                "analysis_type": (
                    analysis_type if "analysis_type" in locals() else "cycles"
                ),
            },
        )
        return JSONResponse(
...
async def analyze_rapid_movements(
    ...
    try:
        logger.info(
            f"Starting rapid movement analysis for {len(request.entity_ids)} entities"
        )

        # Validate that this is single entity analysis (based on requirements)
        if len(request.entity_ids) > 1:
            raise ValidationError(
                "Rapid movement analysis currently supports single entity analysis only"
            )
        ...
        # Determine success message based on results
        rapid_movements_found = results.get("results", {}).get(
            "rapid_movements_found", False
        )
        ...
        logger.info(
            f"Rapid movement analysis completed successfully in {processing_time_ms}ms"
        )
        ...
        error_response = ErrorResponse(
            error_code="INTERNAL_ERROR",
            message="An unexpected error occurred during analysis.",
            details={"analysis_type": "rapid_movements"},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
...
async def analyze_time_trends(
    ...
    try:
        logger.info(
            f"Starting time trends analysis for {len(request.entity_ids)} entities"
        )

        # Validate that this is single entity analysis (based on requirements)
        if len(request.entity_ids) > 1:
            raise ValidationError(
                "Time trends analysis currently supports single entity analysis only"
            )
        ...
        trends_detected = bool(
            trend_analysis.get("overall_assessment", {}).get("debit_trend_strength", 0)
            > 0.3
            or trend_analysis.get("overall_assessment", {}).get(
                "credit_trend_strength", 0
            )
            > 0.3
        )
        ...
        logger.info(
            f"Time trends analysis completed successfully in {processing_time_ms}ms"
        )
        ...
        error_response = ErrorResponse(
            error_code="ENTITY_NOT_FOUND",
            message=str(e),
            details={"entity_ids": request.entity_ids, "analysis_type": "time_trends"},
        )
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content=error_response.model_dump(mode="json"),
        ...
        error_response = ErrorResponse(
            error_code="ANALYSIS_ERROR",
            message="Analysis processing failed. Please check your request and try again.",
            details={"analysis_type": "time_trends", "entity_ids": request.entity_ids},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
        ...
        error_response = ErrorResponse(
            error_code="INTERNAL_ERROR",
            message="An unexpected error occurred during analysis.",
            details={"analysis_type": "time_trends"},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
...
async def analyze_transfer_patterns(
    ...
    try:
        logger.info(
            f"Starting transfer pattern analysis for {len(request.entity_ids)} entities"
        )

        # Validate that this is multiple entity analysis (based on requirements)
        if len(request.entity_ids) < 2:
            raise ValidationError(
                "Transfer pattern analysis requires multiple entities (minimum 2)"
            )
        ...
        # Determine success message based on results
        patterns_found = results.get("results", {}).get(
        ...
        logger.info(
            f"Transfer pattern analysis completed successfully in {processing_time_ms}ms"
        )
        ...
        error_response = ErrorResponse(
            error_code="INTERNAL_ERROR",
            message="An unexpected error occurred during analysis.",
            details={"analysis_type": "transfer_patterns"},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=error_response.model_dump(mode="json"),
